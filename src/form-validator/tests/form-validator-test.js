...
YL.augmentObject(YW.formValidatorTests,{
    formValidatorTestCase:new YAHOO.tool.TestCase({
        ...
        testCheckOnSubmit:function(){
            ...
            YD.get('test-text-input').value = '';
            validator.validate();
            YD.get('test-text-input').value = 'Some value';
            validator._onFormInteraction(null, YD.get('test-text-input'));
            Assert.areEqual('', this.incorrectEl.style.display);
            Assert.areEqual('none', this.correctEl.style.display);
        ...
        /**
         * This will test to ensure that turning an input on/off will update the submit button properly
         */
        testTurnOffOnInput:function(){
            var Assert = YAHOO.util.Assert,validator;
            // create the validator with one input and one button
            validator = new YW.FormValidator(this.hiddenDiv,{
                fields:{
         ...
         * This will test to ensure that turning an input on/off will update the submit button properly
         */
        testTurnOffOnGroup:function(){
            var Assert = YAHOO.util.Assert,validator, groupCorrect = document.createElement('DIV'), groupIncorrect = document.createElement('DIV');
            //this.subHiddenDiv.appendChild(groupCorrect);
            //this.subHiddenDiv.appendChild(groupIncorrect);
            // create the validator with one input and one button
            validator = new YW.FormValidator(this.hiddenDiv,{
                fields:{
                    'test-group':{
            ...
            Assert.areEqual('none', this.incorrectEl.style.display);
            Assert.areEqual('', this.correctEl.style.display);
            Assert.areEqual('none', groupIncorrect.style.display);
            Assert.areEqual('', groupCorrect.style.display);
            ...
            Assert.areEqual('none', this.incorrectEl.style.display);
            Assert.areEqual('none', this.correctEl.style.display);
            Assert.areEqual('', groupIncorrect.style.display);
            Assert.areEqual('none', groupCorrect.style.display);
